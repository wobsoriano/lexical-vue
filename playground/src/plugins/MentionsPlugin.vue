...
watch(queryString, (mentionString) => {
...
class MentionTypeaheadOption extends MenuOption {
  ...
  constructor(name: string, picture: Component) {
    super(name)
    this.name = name
...
const options = computed(
  () =>
    results.value
      .map(
        result =>
          new MentionTypeaheadOption(result, h('i', { class: 'icon user' })),
      )
...
function onSelectOption({
  ...
  textNodeContainingQuery: nodeToReplace,
  closeMenu,
}: {
...
<template>
  <LexicalTypeaheadMenuPlugin
    ...
    :trigger-fn="checkForMentionMatch"
    :options="options"
    @query-change="queryString = $event"
    @select-option="onSelectOption"
  >
    <Teleport v-if="anchorElementRef && results.length" :to="anchorElementRef">
