    ...
    const fn = mergeRegister(
      editor.registerNodeTransform(TextNode, (textNode: TextNode) => {
        ...
        if ($isAutoLinkNode(parent) && !parent.getIsUnlinked()) {
          handleLinkEdit(parent, toValue(matchers), onChangeWrapped)
        }
          ...
          if (
            ...
            $handleLinkCreation(textNodesToMatch, toValue(matchers), onChangeWrapped)
          }

          handleBadNeighbors(textNode, toValue(matchers), onChangeWrapped)
        }
      }),
...
function getTextNodesToMatch(textNode: TextNode): TextNode[] {
  // check if next siblings are simple text nodes till a node contains a space
