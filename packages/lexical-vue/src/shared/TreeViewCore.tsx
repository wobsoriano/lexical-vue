...
export const TreeViewCore = defineComponent(
  (props: LexicalTreeViewProps, ctx: { expose: (exposed: object) => void }) => {
    ...
    return () => (
      <div class={props.viewClassName}>
        ...
        {showLimited.value || !isLimited.value ? <pre ref={preRef}>{content.value}</pre> : null}

        {timeTravelEnabled.value && (showLimited.value || !isLimited.value) ? (
